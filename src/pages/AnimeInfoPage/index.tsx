...
function goToEpisode(url: string, navigate: NavigateFunction) {
  console.log(url);
  navigate("/watchEpisode");
}

function AnimeInfoPage({ getAnimeInfo }: AnimeInfoPageProps) {
  const navigate = useNavigate();
  if (getAnimeInfo.title == undefined) {
    navigate('/');
    scrollTo(0, 0);
  }
    ...
    <body className='infoBody'>
      ...
      <div className='episodes'>
        {getAnimeInfo.episodes.map((episode: string[]) => {
          return (
            <div key={getAnimeInfo.pageurl} className='episode' onClick={() => { goToEpisode(getAnimeInfo.pageurl, navigate) }}>
              <h3>{episode[0][0]}</h3>
              <p>{episode[0][2]}</p>
            </div>
          )
